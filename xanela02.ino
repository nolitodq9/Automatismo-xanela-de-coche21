...
int estado = 1;       // o valor é un numero enteiro, pode ser 0,1,2,3 ... Come
int contador = 0;    // contador para o numero de impulsos do motor; modificamos este parámetro para que o monitor serie non conte mentras non premamos o botoón.


void setup() {            
                            // Aquí definimos os pins de Arduíno, sexan entradas ou saídas
  
  pinMode(motorArriba, OUTPUT);     // Aquí explicámolle a Arduíno que o pin motor arriba é unha saída
  pinMode(motorAbaixo, OUTPUT);     // Aquí explicámolle a Arduíno que o pin motor abaixo é unha saída
...
void loop() {               // Esta parte é na que se executa o código "bucle"
  ...
  if(digitalRead(pulsador)) {          // Aquí escribimos unha condición (if), na que dicimos " si a lectura dixital (pulsador)" pase as seguintes accións:
    estado= (++estado) % 4;            // Actuaizacion do estado para estar entre 0 e 3, no que temos 3 estados; motorArriba,paro, motorAbaixo.. Empregamos a suma e a división entre catro para obter números inferiores a 4 e así poder pasar sucesivamente dun estado a outro.
